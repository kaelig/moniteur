  ...
  init () {
    return Object.keys(this.assets).map(assetName =>
      new Promise((resolve, reject) => {
        ...
        if (!assetEndpoints) {
          reject(console.error('No asset names were found'))
        }

        Object.keys(sensors[assetType]).map(metric => {
          resolve(log('Set up recorder:', assetName, metric))
      ...
      switch (assetType) {
        case 'js':
          ...
          return new Promise((resolve, reject) => {
            ...
            return jsparser.parse()
              .then(stats =>
                Object.keys(sensors['js']).map(metric =>
                  this.recorders[assetHash][metric](stats[metric], () =>
                    resolve(log(`Recorded: ${assetHash} 'js' ${assetEndpoints} ${metric} ${stats[metric]}`))
                  )
                )
              )
        ...
        case 'css':
          return new Promise((resolve, reject) => {
            ...
            stats.parse()
              .then(result =>
                Object.keys(sensors['css']).map(metric =>
                  this.recorders[assetHash][metric](result[metric], () =>
                    resolve(log(`Recorded: ${assetHash} css ${assetEndpoints} ${metric} ${result[metric]}`))
                  )
                )
              )
              .catch(reason => reject(reason))

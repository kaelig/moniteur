...
function setCSSRecorders(stylesheets) {
  // Create indexes in the database
  return _.map(_.keys(stylesheets), function(stylesheet) {
    return new Promise(function(resolve, reject) {
      assets[assetKey(stylesheet)] = {};

      _.each(config.cssmetrics, function(name, metric) {
        var key = 'stylesheets.' + assetKey(stylesheet) + '.' + metric;
        lemdb.index(key, name);
        assets[assetKey(stylesheet)][metric] = lemdb.recorder(key);
      });
    });
  });
}
    ...
    return new Promise(function(resolve, reject) {
      stats.parse(function (error, result) {
        ...
        _.each(config.cssmetrics, function(name, metric) {
          ...
          assets[assetKey(stylesheet)][metric](result[metric]);
        });
      });
    });
  });
...
var recorders = setCSSRecorders(config.stylesheets);

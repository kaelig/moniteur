...
function getAssetMetrics(category, assethash, args) {
  ...
  options = args ? _.defaults(args, config) : config;
...
function getAssetSeries(category, assethash, args) {
  var options = {
    start: null,
    end: null,
    metrics: null
  ...
  return _.map(_.keys(options.metrics), function(metric) {
    return new Promise(function(resolve, reject) {
      ...
      lemdb.valuestream(
        ...
        resolve({ name: options.metrics[metric], data: series[assethash][metric].data });
      }));
    });
  });
}
...
function getCSSSeries(stylesheets) {
  return _.map(stylesheets, function(stylesheet) {
    return new Promise(function(resolve, reject) {
      var series = getAssetSeries('stylesheets', md5(stylesheet), { metrics: config.cssmetrics });
